...
from main_win.YOLOv5GUI import Ui_mainWindow
...
class DetThread(QThread):  # 目标检测类继承自多线程QThread
    ...
    # 发出：检测暂停停止完成错误消息 emit：detecting/pause/stop/finished/error msg
    ...
    def __init__(self):
        ...
        self.iou_thres = 0.45
        self.jump_out = False  # 跳出循环jump out of the loop
        self.is_continue = True  # 继续暂停 continue/pause
        self.percent_length = 1000  # 进度条 progress bar
    ...
    def run(self,
            ...
            save_crop=False,  # 保存裁剪的预测框 save cropped prediction boxes
            nosave=False,  # 不保存图像视频 do not save images/videos
            classes=None,  # 按类筛选：--类 0，或 --类 0 2 3 filter by class: --class 0, or --class 0 2 3
            agnostic_nms=False,  # 与类无关的 NMS class-agnostic NMS
            augment=False,  # 增强推理 augmented inference
            visualize=False,  # 可视化要素 visualize features
            update=False,  # 更新所有模型 update all models
            project='runs/detect',  # 将结果保存到项目名称 save results to project/name
            name='exp',  # 将结果保存到项目名称 save results to project/name
            exist_ok=False,  # 现有项目名称正常，不递增 existing project/name ok, do not increment
            line_thickness=3,  # 边界框厚度（像素） bounding box thickness (pixels)
            hide_labels=False,  # 隐藏标签 hide labels
            hide_conf=False,  # 隐藏信心 hide confidences
            half=False,  # 使用 FP16 半精度推理 use FP16 half-precision inference
            ):

        # 初始化 Initialize
        try:
            device = select_device(device)
            half &= device.type != 'cpu'  # 仅在 CUDA 上支持半精度 half precision only supported on CUDA

            # 模型加载 Load model  调用模型代码
            model = attempt_load(self.weights, map_location=device)  # 加载 FP32 型号 load FP32 model
            num_params = 0
            for param in model.parameters():
                num_params += param.numel()
            stride = int(model.stride.max())  # 模型步幅 model stride
            imgsz = check_img_size(imgsz, s=stride)  # 检查图像大小 check image size
            names = model.module.names if hasattr(model, 'module') else model.names  # 获取类名 get class names
            if half:
                model.half()  # to FP16
            ...
            if self.source.isnumeric() or self.source.lower().startswith(('rtsp://', 'rtmp://', 'http://', 'https://')):
                ...
                cudnn.benchmark = True  # 设置 True 以加快恒定图像大小推理 set True to speed up constant image size inference
            ...
            while True:
                ...
                if self.is_continue:
                    ...
                    pred = non_max_suppression(pred, self.conf_thres, self.iou_thres, classes, agnostic_nms,
                                               max_det=max_det)
                    # 过程检测解析结果  Process detections
                    for i, det in enumerate(pred):  # 每个图像的检测数 detections per image
                        im0 = im0s.copy()
                        annotator = Annotator(im0, line_width=line_thickness, example=str(names))
                        if len(det):
                            # 将框从 img_size 大小重新缩放到 im0 大小 Rescale boxes from img_size to im0 size
                            det[:, :4] = scale_coords(img.shape[2:], det[:, :4], im0.shape).round()

                            # 写入结果 Write results
                            for *xyxy, conf, cls in reversed(det):
                                c = int(cls)  # 整数类 integer class
...
class MainWindow(QMainWindow, Ui_mainWindow):
    def __init__(self, parent=None):
        ...
        # 自动搜索模型 search models automatically
        self.comboBox.clear()
    ...
    def chose_cam(self):
        try:
            self.stop()
            MessageBox(
                self.closeButton, title='提示', text='加载摄像头Loading camera', time=2000, auto=True).exec_()
            # 获取本地摄像机的数量 get the number of local cameras
    ...
    def show_image(img_src, label):
        try:
            ...
            # 保持原始纵横比 keep original aspect ratio
            if iw / w > ih / h:
                scal = w / iw
                nw = w
                nh = int(scal * ih)
